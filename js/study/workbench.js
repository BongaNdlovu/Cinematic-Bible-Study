...
(function () {
  ...
  // --- COMPONENT 5: Final Course Capstone ("Prophetic Chain Synthesis") ---
  function renderFinalCapstoneModal(onGraduated) {
    const existingModal = document.getElementById('final-capstone-modal');
    if (existingModal) existingModal.remove();

    const modal = document.createElement('div');
    modal.id = 'final-capstone-modal';
    modal.className = "fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-black/85 backdrop-blur-md overflow-y-auto";
    modal.innerHTML = `
      <div class="bg-paper-50 dark:bg-paper-950 border-2 border-amber-600/70 rounded-2xl max-w-3xl w-full max-h-[92vh] overflow-y-auto flex flex-col shadow-2xl p-6 sm:p-8 font-sans">
        <div class="flex items-center justify-between border-b border-paper-300 dark:border-paper-800 pb-4 mb-5">
          <div class="flex items-center gap-3">
            <span class="text-3xl">🎓</span>
            <div>
              <span class="font-mono text-xs uppercase tracking-widest text-amber-700 dark:text-amber-400 font-bold block">Course Graduation Capstone</span>
              <h2 class="font-serif text-2xl font-bold text-ink-900 dark:text-paper-100">The Prophetic Chain Defense: Full Synthesis</h2>
            </div>
          </div>
          <button type="button" id="final-capstone-close-btn" class="text-ink-500 hover:text-ink-900 dark:text-paper-400 p-2 font-mono text-xs">✕ Close</button>
        </div>

        <p class="text-xs text-ink-600 dark:text-paper-400 leading-relaxed mb-6">
          To receive your verified graduation certificate and graduate from the Historicist Scroll of Daniel, you must synthesize the unbroken timeline from 605 B.C. to 1844 A.D.
        </p>

        <!-- Chronology Matching -->
        <div class="p-5 rounded-xl border border-paper-300 dark:border-paper-800 bg-paper-100/50 dark:bg-paper-900/50 mb-5">
          <h3 class="font-serif text-base font-bold text-ink-900 dark:text-paper-100 mb-2">
            Part 1: The Prophetic Chronology Chain
          </h3>
          <p class="text-xs text-ink-700 dark:text-paper-300 mb-4">
            Match the biblical and historical anchor dates to their prophetic fulfillments:
          </p>
          <div class="space-y-3 text-xs">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 p-2.5 rounded bg-paper-50 dark:bg-paper-950 border border-paper-300 dark:border-paper-800">
              <span class="font-mono font-bold text-amber-800 dark:text-amber-300">605 B.C. &bull; Nebuchadnezzar's Dream</span>
              <select id="fc-date-605" class="p-1.5 rounded border border-paper-300 dark:border-paper-700 bg-white dark:bg-paper-900 text-xs">
                <option value="">Select fulfillment...</option>
                <option value="gold">Babylon established as the Golden Head</option>
                <option value="stone">The stone strikes the feet</option>
                <option value="decree">Artaxerxes issues the decree</option>
              </select>
            </div>
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 p-2.5 rounded bg-paper-50 dark:bg-paper-950 border border-paper-300 dark:border-paper-800">
              <span class="font-mono font-bold text-amber-800 dark:text-amber-300">457 B.C. &bull; Decree of Artaxerxes</span>
              <select id="fc-date-457" class="p-1.5 rounded border border-paper-300 dark:border-paper-700 bg-white dark:bg-paper-900 text-xs">
                <option value="">Select fulfillment...</option>
                <option value="fall">Fall of Babylon to Cyrus</option>
                <option value="anchor">Starting point of the 70 weeks and 2,300 days</option>
                <option value="stone">Rome divides into ten toes</option>
              </select>
            </div>
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 p-2.5 rounded bg-paper-50 dark:bg-paper-950 border border-paper-300 dark:border-paper-800">
              <span class="font-mono font-bold text-amber-800 dark:text-amber-300">A.D. 538 to 1798 &bull; 1,260 Prophetic Days</span>
              <select id="fc-date-1260" class="p-1.5 rounded border border-paper-300 dark:border-paper-700 bg-white dark:bg-paper-900 text-xs">
                <option value="">Select fulfillment...</option>
                <option value="supremacy">Papal civil supremacy from Ostrogothic siege to Berthier in Rome</option>
                <option value="alexander">Reign of Alexander the Great</option>
                <option value="exile">Seventy years of Babylonian exile</option>
              </select>
            </div>
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 p-2.5 rounded bg-paper-50 dark:bg-paper-950 border border-paper-300 dark:border-paper-800">
              <span class="font-mono font-bold text-amber-800 dark:text-amber-300">1844 A.D. &bull; Daniel 8:14</span>
              <select id="fc-date-1844" class="p-1.5 rounded border border-paper-300 dark:border-paper-700 bg-white dark:bg-paper-900 text-xs">
                <option value="">Select fulfillment...</option>
                <option value="cleansing">Close of the 2,300 days and Heavenly Sanctuary vindication (Nitsdaq)</option>
                <option value="first">Destruction of the first temple</option>
                <option value="corinth">Paul reaches Corinth</option>
              </select>
            </div>
          </div>
        </div>
        ...
        <div class="p-5 rounded-xl border border-paper-300 dark:border-paper-800 bg-paper-100/50 dark:bg-paper-900/50 mb-5">
          <h3 class="font-serif text-base font-bold text-ink-900 dark:text-paper-100 mb-2">
            Part 2: The Apologist's Synthesis Defense
          </h3>
          <p class="text-xs text-ink-700 dark:text-paper-300 mb-3 italic">
            In your own words, summarize why the historicist school views the prophetic chain of Daniel as contiguous, unbroken history rather than past-only (Preterism) or future-gap (Futurism):
          </p>
          <textarea id="final-capstone-text" class="w-full min-h-[90px] p-3 rounded-lg border border-paper-300 dark:border-paper-700 bg-paper-50 dark:bg-paper-950 text-xs text-ink-900 dark:text-paper-100 font-sans leading-relaxed" placeholder="Draft your synthesis defense... (e.g., The historicist method recognizes the continuous fulfillment of prophecy from Daniel's day to the second advent. The year-day principle connects 457 B.C. through A.D. 27 and 34 to 1844, proving that prophecy is contiguous without arbitrary multi-century gaps.)"></textarea>
        </div>

        <div id="final-capstone-feedback" class="hidden p-4 rounded-xl mb-4 text-xs font-medium"></div>

        <div class="flex items-center justify-between pt-4 border-t border-paper-300 dark:border-paper-800">
          <div class="text-xs text-ink-500 font-mono">
            Requires: 4 chronology matches &bull; continuous historicist defense
          </div>
          <button type="button" id="final-capstone-submit-btn" class="px-6 py-3 rounded-lg bg-amber-600 hover:bg-amber-700 text-paper-50 font-sans font-bold text-xs shadow-md transition-colors flex items-center gap-2">
            <span>Verify Defense & Unlock Certificate</span> &rarr;
          </button>
        </div>
      </div>
    `;
    ...
    modal.querySelector('#final-capstone-close-btn').addEventListener('click', () => modal.remove());

    modal.querySelector('#final-capstone-submit-btn').addEventListener('click', () => {
      const d605 = modal.querySelector('#fc-date-605').value;
      const d457 = modal.querySelector('#fc-date-457').value;
      const d1260 = modal.querySelector('#fc-date-1260').value;
      const d1844 = modal.querySelector('#fc-date-1844').value;
      const defenseText = (modal.querySelector('#final-capstone-text').value || '').trim().toLowerCase();
      const fb = modal.querySelector('#final-capstone-feedback');

      fb.classList.remove('hidden');

      const matchesOk = d605 === 'gold' && d457 === 'anchor' && d1260 === 'supremacy' && d1844 === 'cleansing';
      const textOk = defenseText.length >= 25 && (/contigu|unbroken|year-day|continuous|chain|gap|histor/i.test(defenseText));

      if (matchesOk && textOk) {
        fb.className = "p-4 rounded-xl mb-4 text-xs font-sans leading-relaxed border border-emerald-500/60 bg-emerald-50 dark:bg-emerald-950/40 text-emerald-950 dark:text-emerald-200";
        fb.innerHTML = "<strong>✓ Graduation Defense Verified!</strong> You have demonstrated full mastery of the historicist prophetic chain from Babylon to the sanctuary vindication.";
        localStorage.setItem('ba_capstone_final_mastered', 'true');
        setTimeout(() => {
          modal.remove();
          if (typeof onGraduated === 'function') onGraduated();
        }, 1200);
      } else {
        fb.className = "p-4 rounded-xl mb-4 text-xs font-sans leading-relaxed border border-rose-500/60 bg-rose-50 dark:bg-rose-950/40 text-rose-950 dark:text-rose-200";
        const errors = [];
        if (!matchesOk) errors.push("• Please check your date matches. All 4 prophetic anchors must be correctly paired.");
        if (!textOk) errors.push("• Please explain how the continuous, unbroken historicist chain differs from gap-based theories.");
        fb.innerHTML = `<strong>Synthesis Incomplete:</strong><br>${errors.join('<br>')}`;
      }
    });
  }
  ...
  window.StudyWorkbench = {
    ...
    renderFinalCapstoneModal: renderFinalCapstoneModal,
