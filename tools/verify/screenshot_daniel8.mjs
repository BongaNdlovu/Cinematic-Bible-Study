...
try {
  ...
  const shots = [
    { id: 'ram', title: 'Ram of Medo-Persia', out: 'qa/proofs/gallery_ram.png' },
    { id: 'goat', title: 'Charging Goat', out: 'qa/proofs/gallery_goat.png' },
    { id: 'goat_broken', title: 'Broken Horn', out: 'qa/proofs/gallery_goat_broken.png' },
    { id: 'goat_horn', title: 'Little Horn of the Goat', out: 'qa/proofs/gallery_goat_horn.png' },
  ];
  ...
  for (const b of shots) {
    ...
    console.log(b.id, info, 'load', state.title, state.geom);
  ...
  await sleep(1400);
  await client.eval(`document.querySelectorAll('.artifact-grid.four')[1]?.scrollIntoView({block:'center'})`);
  await sleep(400);
  const n = await client.eval(`document.querySelectorAll('a[href="gallery.html?asset=ram"]').length`);
  console.log('ram tiles on cover', n);
  if (!n) throw new Error('Daniel 8 ram tile missing on cover');
  await client.shot('qa/proofs/index_daniel8.png');
